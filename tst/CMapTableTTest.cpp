...
typedef CMapTableT<TItem, 50, 600> CItemList;

TEST_GROUP(CMapTableTGrp)
{
  void setup()
  {
...
TEST(CMapTableTGrp, add1del1)
{
   ...
   list.del(index);
   POINTERS_EQUAL(list.get(index), NULL);
   POINTERS_EQUAL(list.m_maps[index], NULL);
   CHECK_EQUAL(0, list.m_allocNum);
}

TEST(CMapTableTGrp, add50del50)
{
   CItemList list;
   unsigned int index = 0;
   TItem item = {0};
...
TEST(CMapTableTGrp, add50while_del)
{
   CItemList list;
   unsigned int index = 0;
...
TEST(CMapTableTGrp, miscItem)
{
   ...
   POINTERS_EQUAL(pItem, list.m_maps[index]);
   TItem item = {3, 2, 1};
   list.set(index, item);
